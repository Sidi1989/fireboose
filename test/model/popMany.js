...
describe('Model', function () {
  ...
  describe('#popMany()', function () {
    it('should remove the last element of an arrayProp from multiple Docs, without error', function (done) {
      const newQuery = new Query()
        .where('founder', '==', 'France');

      UnindexedCountry.popMany(newQuery, 'languages')
        .then(function (resolve) {
          let expectedResolve = ['country12', 'country13', 'country14'];
          if (_.isEqual(expectedResolve, resolve)) {
            done()
          } else {
            done(new Error('Failure in #popMany()'))
          }
        })
